...
public class FoodControllerImpl implements FoodController {
    ...
    public AjaxResult addFood(@RequestBody Food food) {
        ...
        if(typeName.equals("")){
            return AjaxResult.build("0","平台分类不能为空");
        }
        Map map =new HashMap();
        map.put("name",typeName);
        List<Type> list =(List<Type>)typeService.select(map).getObj();//当前list中只可能有一个存在
        ...
        if(foodTypeName.equals("")){
            return AjaxResult.build("0","店铺分类不能为空");
        }
        Map foodTypeMap=new HashMap();
        foodTypeMap.put("shopId",shop.getId());
        foodTypeMap.put("name",foodTypeName);
        List<FoodType> foodTypeList=(List<FoodType>) foodTypeService.select(foodTypeMap).getObj();//当前list中只可能有一个存在
    ...
    public AjaxResult findAllType( ) {
        ...
        Map map=new HashMap();
        map.put("canUse",true);
        List<Type> listFood = (List<Type>) typeService.select(map).getObj();
        map = new HashMap();
     ...
     */
    @Override
    @PostMapping("updateFoodFormat")
    public AjaxResult updateFoodFormat(@RequestBody FoodFormat foodFormat) {
        Shop shop=shopService.getShopByUser();
        if(shop==null){
    ...
    public AjaxResult findAllFood(){
        if (RedisUtil.exists("foodList")){
            log.info("从缓存中获商品信息..");
            List list= JSON.parseObject(RedisUtil.get("foodList"),List.class);
            if (list==null){
        ...
        List<Food> foodList =(List<Food>) bll.selectAll().getObj();
        Object obj=foodList;
        RedisUtil.setEx("foodList",JSON.toJSONString(obj),60*60*12);//12小时
        log.info("已经缓存。。。。。");
        return AjaxResult.build("1","查询成功！",foodList);
    }
     ...
     * @return
     */
    @RequestMapping("findAllFoodBySearchName")
    public AjaxResult findAllFoodBySearchName(@RequestBody String searchName){
        Map map=new HashMap();
        map.put("name",searchName);
        List<Food> foodList=(List<Food>) bll.select(map).getObj();
        return AjaxResult.build("1","查询成功！",foodList);
    }
