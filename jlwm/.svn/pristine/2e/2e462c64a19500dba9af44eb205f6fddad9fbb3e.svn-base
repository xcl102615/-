    ...
    $(function () {
      ...
      var time = setInterval(function () {
        ...
        function taskUI(task) {
            ...
            if (task != null) {
                str += '<tr>\n' +
                    ...
                    '                  <td><button type="button" id='+task.order.id+' class="btn btn-success  btn-xs">查看详情</button>&nbsp;<button type="button" status='+task.status+' value='+task.id+'  class="btn btn-danger  btn-xs">删除订单</button></td>\n' +
                    '                </tr>'
                return str;
            }
        }
        ...
        $("body").on("click","button.btn-danger",function () {
            var id=$(this).attr("value")//id值
            var status=$.trim($(this).attr("status"));//去除空格的状态值
            var deliveryOrder=new Object();
            deliveryOrder.id=id;
            if(status=="已完成"){
                if(confirm("确定删除该订单?")){
                    //点击确定后操作
                    doDataAll("<%=basePath%>/deliveryOrder/delDeliveryOrder",deliveryOrder,function(data){
                        if(data.code>0){
                            alert("删除成功");
      ...
      doDataAllAsc("<%=basePath%>/user/getCurrentUser","",function (data) {
        ...
        param.user=user
          doDataAllAsc("<%=basePath%>/deliveryOrder/findDeliveryOrder",param,function (data) {
              console.info(data)
              $(data.obj).each(function () {
