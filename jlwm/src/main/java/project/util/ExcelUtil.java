...
public class ExcelUtil {
    ...
    public static void main(String[] args) {
        ...
        String src_xlspath = "D:\\POITest.xlsx";
        String dist_xlsPath = "D:\\POI.xlsx";
        List<Row> rowList;
        try {
    ...
    public void writeExcel(List<Row> rowList, Workbook wb, OutputStream to) throws IOException {
    ...
    private void writeExcel(Workbook wb, List<Row> rowList, OutputStream to) {
    ...
    /*根据列名称获取列的索引*/
    public int getColIndex(Row headerRow, String colName) {
        if (headerRow == null) return -1;
        java.util.Iterator<Cell> items = headerRow.iterator();
        while (items.hasNext()) {
            Cell item = items.next();
            if (item.getStringCellValue().trim().equals(colName.trim()))
