...
public class DeliveryOrderServiceImpl extends BaseService implements DeliveryOrderService {
    ...
    public AjaxResult update(Object object) {
        ...
        List<OrderDetails> orderDetailsList=orderPrice.getOrderDetails();//获取到当前订单的所有订单详情
        if(orderDetailsList.size()>0){
            for(OrderDetails newOrderDetalis:orderDetailsList){
                orderService.updateOrderDetails(newOrderDetalis);//修改订单详情的状态
            }
        }
        //新增骑手流水
        ...
        deliveryMoney.setName(((Order)orderService.selectOneById(order.getId()).getObj()).getName());
