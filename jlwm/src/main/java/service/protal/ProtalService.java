...
public class ProtalService {
    ...
    public AjaxResult getIndexInfo(){
        if (RedisUtil.exists("indexInfo")){
            log.info("从缓存中获取首页数据..");
            Map rs=JSON.parseObject(RedisUtil.get("indexInfo"),Map.class);
...
//        查询所有启用的轮播图
        ...
        param.clear();
        param.put("orderBy","num");
//         最低消费
        param.put("isOpen","1");
        //         是否在营业
        param.put("topX",20);
        param.put("canUse",true);
//        topX 前几条数据 使用方法 没这个参数会报错
        ...
        RedisUtil.setEx("indexInfo", JSON.toJSONString(rs),300);
        return AjaxResult.build("1","系统响应了你的请求",rs);
    }
